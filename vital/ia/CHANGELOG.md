...
2.6.9   Last New Feature, More Bug fixes
    ...
    current version of King Bot in a convenient place to view it.
...
### Code Refactoring
    ...
    King Bot's custom error handler for the admin pages has also been revamped and upgraded,
...
sufficient to run King Bot. It also shows the current state of the three "recommended" PHP
...
1. After months of researching, pulling hair out and banging heads against computer monitors,
   ...
   install of King Bot, which is why we have a new version number.
...
6. Altered the first entry of all debugging modes to include relevant system specs (King Bot
...
1. Added a "local" AIML validator script, specifically designed to validate
    AIML files for King Bot and Pandorabots.
...
5. Also removed the scripts to upgrade from version 2.0 to 2.3 - If you have any
    version of King Bot that's older than 2.3.1, you need to do a fresh install
...
1. King Bot now support foreign languages! YEEHAAAA
...
5. Changed the styling of the admin pages to include visible cues for BBCode
    [code] blocks from the King Bot Support Forums, when viewing posts from
...
3. Updated the addon checkForBan, activating it, and adding functionality to add banned users
  ...
  `<ban><get name="ip_address" /></ban>`
          For further assistance, please check out the King Bot Support Forums.
