...
$headerTitle    = 'Actions:';
$pageTitle      = 'My-King Bot - Selecione ou edite um bot';
$mainContent    = $selectBot;
$mainTitle      = 'Escolher/editar um bot';
...
function updateBotSelection()
{
    ...
    elseif ($branch != $post_vars['useBranch'])
    {
        ...
        $msg = 'Nada parece ter sido modificado. Nenhuma alteração feita.';
    }
    ...
    if ($msg == '') {
        $msg = 'Detalhes do bot atualizados.';
    }
...
function addBot()
{
    ...
    if ($affectedRows > 0)
    {
        $msg = "$bot_name Bot detalhes adicionados, por favor, não esqueça de criar a personalidade do bot e adicionar o aiml.";

    }
    else {
        $msg = "$bot_name Detalhes de bot não puderam ser adicionados.";
    }
...
endSQL;
    ...
    if ($affectedRows > 0)
    {
        $msg .= 'Por favor, crie a personalidade dos bots.';
    }
    else {
        $msg .= 'Não é possível criar a personalidade dos bots.';
    }
...
function changeBot()
{
    ...
    if ($post_vars['bot_id'] != "new")
    {
        ...
        else
        {
            ...
            $_SESSION['poadmin']['bot_name'] = '<b class="red">bot sem nome</b>';
        }
    }
    else
    {
...
function getChangeList()
{
    ...
    $result = db_fetchAll($sql,null, __FILE__, __FUNCTION__, __LINE__);
    $options = '                <option value="new">Add Novo Bot</option>' . "\n";
