	...
	public function register() {
		...
		if ($user_id = $this->User_Model->save()) {
			...
			if($this->settings['users_must_confirm_email'] == 1){
				...
				$message .= "<br/><br/><br/>".$this->lang->s("Please do not reply to this email.");
	...
	public function account_recovery(){
		...
		if ($this->form_validation->run() == false) {
			...
			if(count($user)){
				...
				$message = $this->lang->s("Hello %s",$user->firstname).",<br/><br/>";
				$message .= $this->lang->s("You recently requested a new password")."<br/><br/>";
				...
				$this->email->reply_to(ADMIN_EMAIL, $this->settings['sitename']);
				$this->email->to($this->input->post('email',TRUE));
				$this->email->subject($this->lang->s('Account recovery request'));
				$this->email->message($message);
