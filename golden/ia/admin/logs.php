...
$convo = (isset ($get_vars['id'])) ? getuserConvo($get_vars['id'], $show) : "Por favor, selecione uma conversa na barra lateral.";
...
$headerTitle = 'Actions:';
$pageTitle = 'My-King Bot - Chat Logs';
...
function getUserList($bot_id, $showing)
{
    ...
    switch ($showing)
    {
        ...
        break;
        case 'last 20':
        $sql = "SELECT DISTINCT(`user_id`),COUNT(`user_id`) 
        AS TOT 
        FROM `conversation_log`  
        WHERE  bot_id = '$bot_id' 
        GROUP BY `user_id` 
        ORDER BY ABS(`user_id`) 
        DESC limit 20;";
            //$repl_date = time();
        $repl_date = false;
        break;
    ...
    if ($numRows == 0 || false === $rows)
    {
        ...
        foreach ($rows as $row)
        {
            ...
            $tmpLink = str_replace('[linkLabel]', "Numero #{$user_id}:$userName($TOT)", $tmpLink);
...
function getuserConvo($id, $showing)
{
    ...
    switch ($showing)
    {
        ...
        $title = "Últimas 20 entradas de conversação para o usuário: $user_name (ID #{$id})";
        $ats = '0 limit 20';
        break;
        case 'all time' :
        $sql = 'SELECT *  FROM `conversation_log` WHERE `bot_id` = :bot_id AND `user_id` = :user_id ORDER BY `id` ASC;';
        $title = "Todas as conversas para o usuário: $user_name (ID #{$id})";
        $ats = 'foo';
        break;
        default :
        $title = "(ERRO! Valor inesperado para exibição: {$showing}!)";
    }
    ...
    $ts = (!empty($ats)) ? $ats : $ts;
    $clearLink = <<<endLink
    <a href="#" class="cl_del" data-sr="[sr]">Limpar logs para esta conversa</a>
endLink;
    ...
    foreach ($result as $row)
    {
        ...
        if ($thisdate != $lasttimestamp)
        {
            ...
            $list .= "<hr><br/><h4>Conversa #$i $thisdate $cl</h4>";
            $i++;
        }
