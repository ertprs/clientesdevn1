...
King Bot is an AIML interpreter written in PHP, and uses a MySQL database to store
chatbot information, including the AIML files used to formulate the chatbot's responses.

## System Requirements

King Bot requires PHP version 5.4.0 or higher to function. It also requires the following
PHP extensions:
...
King Bot supports the creation of multiple chatbots. The individual bots are configured
from the admin area. Bot specific settings and variables are stored in the database.
Configuration of King Bot itself is stored in `/config/global_config.php`.

## Using the bot

After installation you can chat with your King Bot chatbot using several methods:
...
- Through the King Bot API (example in [`/gui/xml/index.php`](gui/xml/index.php))

For advanced botmasters and programmers, it is possible to use the King Bot
...
visit [the King Bot website](http://www.devn1.com).
...
current chatbot, perform a "clean install" of King Bot, and then upload the AIML files
to the new installation. With the release of King Bot version 2.2, the differences
...
installation. Simply point your web browser to your freshly uploaded King Bot directory
...
your new King Bot admin page (e.g. http://www.example.com/ProgramO/admin) and log in using
...
## King Bot has a WordPress Plugin!

If you have a WordPress driven website, and wish to incorporate your King Bot chatbot into your
